<?php
class MfgProduction extends Model implements JsonSerializable{
	public $id;
	...
	public static function filter($name){
		...
		$result=$db->query("select id,production_datetime,bom_id,qty,labor_cost,manager_id,total_cost,product_id,uom_id from {$tx}mfg_productions where name like '%$name%'");
		$data=[];
		while($mfgproduction=$result->fetch_object()){
			$data[]=$mfgproduction;
		}
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		while($mfgproduction=$result->fetch_object()){
			...
			if($action){
				...
				$action_buttons.= Event::button(["name"=>"show", "value"=>"Show", "class"=>"btn btn-info", "route"=>"mfgproduction/show/$mfgproduction->id"]);
				$action_buttons.= Event::button(["name"=>"edit", "value"=>"Edit", "class"=>"btn btn-primary", "route"=>"mfgproduction/edit/$mfgproduction->id"]);
				$action_buttons.= Event::button(["name"=>"delete", "value"=>"Delete", "class"=>"btn btn-danger", "route"=>"mfgproduction/confirm/$mfgproduction->id"]);
	...
	static function html_row_details($id){
		...
		$mfgproduction=$result->fetch_object();
		$html="<table class='table'>";
		$html.="<tr><th colspan=\"2\">MfgProduction Show</th></tr>";
